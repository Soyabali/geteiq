...
/// A reusable ERROR popup used across the whole app.
///
/// Shows a small error Lottie on top, a message below, and closes itself
/// automatically after 4 seconds (tapping outside also closes it).
///
/// Call it in one line from anywhere — e.g. after an API returns result == 0:
///
/// ```dart
/// FailureDialog.show(context, 'Something went wrong');
/// ```
...
class _FailureDialogState extends State<FailureDialog> {
  ...
  void initState() {
    ...
    _autoCloseTimer = Timer(FailureDialog._autoCloseAfter, _close);
  }
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            // Animated error cross. Swap the .json for any Lottie you like —
            // if the file is ever missing it falls back to a red cross so
